...
```{r}
glm.sat <- glm(number ~ factor(tumour.type), family = poisson(log), data=Mela)
matrix(glm.sat$fitted.values,ncol=3)
...
```{r}
glm.sat <- glm(number ~ factor(site), family = poisson(log), data=Mela)
res<-matrix(glm.sat$fitted.values,ncol=3)
res
rowSums(res)
colSums(res)
```
